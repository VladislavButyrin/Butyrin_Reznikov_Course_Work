...
{
    public class MailLogicGuarantor
    {
        ...
        public MailLogicGuarantor(IMessageInfoStorage messageInfoStorage, IOrganizatorStorage
       organizatorStorage)
        {
            _messageInfoStorage = messageInfoStorage;
            _organizatorStorage = organizatorStorage;
        }
        ...
        {
            var client = _organizatorStorage.GetElement(new OrganizatorBindingModel
            {
                Login = model.FromMailAddress
            });
            model.OrganizatorId = client?.Id;
                    ...
                    try
                    {
                       ...
                       SecureSocketOptions.SslOnConnect);
                        ...
                        for (int i = 0; i < client.Count; i++)
                        {
                            ...
                            {
                                int? x = info.OrganizatorStorage.GetElement(new OrganizatorBindingModel { Login = mail.Address })?.Id;
                                info.MessageStorage.Insert(new MessageInfoBindingModel
                                {
                                    OrganizatorId = info.OrganizatorStorage.GetElement(new OrganizatorBindingModel { Login = mail.Address })?.Id,
