...
namespace TravelAgencyView
{
    ...
    public partial class GuidesReportFrame : Window
    {
        ...
        public GuidesReportFrame(ReportLogic report_logic, TripBusinessLogic trip_logic)
        {
            InitializeComponent();
        ...
        private void WordButton_Click(object sender, RoutedEventArgs e)
        {
            using (var dialog = new SaveFileDialog { Filter = "docx|*.docx" })
            {
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    report_logic.SaveExcursionsToWordFile(new ReportBindingModel { FileName = dialog.FileName, Trips = selected_trips });
        ...
        private void ExcelButton_Click(object sender, RoutedEventArgs e)
        {
            using (var dialog = new SaveFileDialog { Filter = "xlsx|*.xlsx" })
            {
                if (dialog.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    report_logic.SaveExcursionsToExcelFile(new ReportBindingModel { FileName = dialog.FileName, Trips = selected_trips });
                    System.Windows.MessageBox.Show("Выполнено", "Успех", MessageBoxButton.OK,
               MessageBoxImage.Information);
                }
            }
